"""Reproduce the managed Skill editing flow initiated by Codex."""
...
FIXTURE_PATH = (
    Path(__file__).parent / "fixtures" / "codex" / "managed-edit-flow.json"
)


class CodexManagedEditFlowTest(unittest.TestCase):
    """Run the exact check -> workspace -> review -> apply Codex sequence."""
    ...
    def setUp(self) -> None:
        ...
        (self.skill / "references" / "codex.txt").write_text(
            "Codex fixture resource\n", encoding="utf-8"
        )
        ...
        self.codex_link = self.codex_home / "skills" / self.fixture["skill"]
        create_directory_link(self.original_deployment, self.codex_link)
        self.target_path = self.codex_link / "SKILL.md"
        self.command_log: list[str] = []

        self.environment = {
            "HOME": str(self.home),
            "CODEX_HOME": str(self.codex_home),
            "WORKBUDDY_HOME": str(self.root / "missing-workbuddy"),
    ...
    def managed_surfaces(self) -> dict[str, object]:
        return {
            ...
            "codex_link_target": str(self.codex_link.resolve()),
        }

    def test_codex_uses_managed_check_and_the_complete_workspace_flow(self) -> None:
        self.assertEqual(self.fixture["schema_version"], 1)
        self.assertEqual(
            self.fixture["target_path"], "$CODEX_HOME/skills/alpha/SKILL.md"
        )
        self.assertEqual(
            self.fixture["forbidden_agent_writes"],
            ["$CANONICAL/**", "$DEPLOYMENT/**", "$CODEX_HOME/skills/alpha/**"],
        )
        self.assertEqual(
        ...
        with mock.patch.dict(
            ...
            side_effect=AssertionError(
                "the Codex managed edit flow must not commit, fetch, or push"
            ),
        ) as git_command:
            ...
            self.assertEqual(ownership["client"], "codex")
            ...
            self.assertEqual(begun["actor"], "codex")
            ...
            # This is the only write performed on behalf of Codex. The CLI owns
            # the later transactional source and deployment replacement.
            workspace_skill = workspace / "SKILL.md"
            self.assertEqual(
                self.fixture["only_agent_write"], "$WORKSPACE/SKILL.md"
            )
            ...
            self.assertEqual(impact["registry_targets"], ["codex"])
            codex = next(
                row for row in impact["clients"] if row["client"] == "codex"
            )
        ...
        self.assertNotEqual(
            str(self.codex_link.resolve()), before["codex_link_target"]
        )
        current_deployment = self.codex_link.resolve()
        verification = verify_deployment(current_deployment)
        self.assertTrue(verification.ok)
        self.assertEqual(verification.provenance["target_client"], "codex")
        self.assertEqual(
