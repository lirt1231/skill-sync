...
| --- | --- | --- | --- |
| CLI synchronization and local Web UI | Verified in CI | Not currently in CI | Experimental; not currently in CI |
...
CI currently runs on macOS only. Linux and Windows validation will return after
their filesystem identity, link, and transactional recovery behavior is
brought to parity with macOS.
